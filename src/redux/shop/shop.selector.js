...
export const selectCollection = memoize((collectionUrlParam) =>  //we pass in the url Params which we use to get which collection to pull off.
    createSelector(
        [selectCollections],
        collections =>  collections ? collections[collectionUrlParam] : null //object notation. We onverted our shop data from an array to an object so that its easier to find the collection we want rather than using collections.find on the array..
    )
)
